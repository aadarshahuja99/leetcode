class Solution {
    public boolean canTransform(String start, String target) {
        ...
        {
            if(start.charAt(sourcePointer) == 'X')
            {
                sourcePointer++;
                continue;
            }
            if(target.charAt(targetPointer) == 'X')
            {
                targetPointer++;
                continue;
            }
        ...
        while(sourcePointer < n && start.charAt(sourcePointer) == 'X')
        {
            sourcePointer++;
        }
        while(targetPointer < n && target.charAt(targetPointer) == 'X')
        {
            targetPointer++;
        }
